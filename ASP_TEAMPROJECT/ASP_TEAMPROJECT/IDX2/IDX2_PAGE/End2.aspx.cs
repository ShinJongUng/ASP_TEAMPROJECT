...
{
    public partial class End2 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (reader.Read())
                {
                    ...
                    Label1.Text = "<font color = skyblue>" + reader["name"].ToString() + "</font> 좋아하는 음식 월드컵 우승";
                }
                else
                {
        ...
        {
            // 1. XML 파일 열기
            string fn = Server.MapPath("~/App_Data/Chat2.xml");
